...
for p=1:1   
    ...
    B=B(row,:) ; 
        ...
        %% Matching
            ...
            triangleTab=ones(size(tri1,1),size(tri2,1),3)*300000;%Tabla con indices de triangulos iguales
                    ...
                    for j=1:size(tri2,1)
                        ...
                        if((((R(i)-Rb(j))^2)<tr(i) + trb(j))  && ((C(i)-Cb(j))^2<tc(i) + tcb(j) && ...
                            ...
                            if( (triangleTab(i,j,2)<tr(i) + trb(j))  &&  triangleTab(i,j,3)<tc(i) + tcb(j))
                                ...
                                if((orientTri1*orientTri2>=0) && R(i)<8 && Rb(j)<8) 
